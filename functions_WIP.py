...
def group_balancing_list (df,col, threshold):
    count_group = df[col].value_counts()
    small_groups = list(count_group[count_group < threshold].index)

    
    return small_groups

    
def group_balancing(x,small_groups):
    if x in small_groups:
        return 'Other'
    else:
        return x
...
def clean_gender(x):
    ...
    males = ['M','Male','Maile','male','Mal','Make','Androgyne','Man','Msle','Mail','Malr','man']
    found_female = False
    found_male = False
    for elem in females:
        if ( elem in x ):
...
def logreg_plots (y_train, y_train_pred, y_test, y_test_pred, model_name):
...
def log_class_scores(model_name, y_train, y_train_pred, y_test, y_test_pred, label):
...
def reg_score_comparison(train_r2_scores, test_r2_scores, model_list):
    ...
    for i in range(0, len(train_r2_scores)):
        ...
        print('The R2 score of the', model_train, ' on Train set is: ', R2_train)
    ...
    for j in range(0, len(test_r2_scores)):
        ...
        print('The R2 score of the', model_test, 'on Test set is: ', R2_test)
